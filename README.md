...
> **Agent skills for context engineering — manage, compress, and optimize LLM context windows**
...
| Concept | Description |
|---|---|
| **Context** | Core primitive of Agent Skills for Context Engineering main — fundamental building block |
| **Engineering** | Execution layer handling engineering operations and state |
| **Agent** | Integration point connecting agent to external services |
| **Skills** | Configuration schema for skills behavior and routing |
| **Window** | Optimization layer reducing window overhead and latency |
...
/skills load Agent-Skills-for-Context-Engineering-main

# Check skill status
/skills list | grep Agent

# Clone and explore locally
gh repo clone hmzainjamil/Agent-Skills-for-Context-Engineering-main
cd Agent-Skills-for-Context-Engineering-main

# Run with Claude Code (invoke skill directly)
claude -p "use Agent Skills for Context Engineering main skill to handle my task"
...
- **Digital agencies** — automate context workflows, cut delivery time 10×
- **SaaS founders** — plug into existing Claude Code setup, zero infra overhead
- **Freelancers** — use as force-multiplier for client work in engineering + agent
...
cp -r . ~/.claude/skills/Agent-Skills-for-Context-Engineering-main/

# Option 3: Via gh CLI
gh repo clone hmzainjamil/Agent-Skills-for-Context-Engineering-main ~/.claude/skills/Agent-Skills-for-Context-Engineering-main
```
...
response = client.messages.create(
    ...
    max_tokens=4096,
    system="You are using the Agent-Skills-for-Context-Engineering-main skill. Agent skills for context engineering — manage, compress, and optimize LLM context windows",
...
mae run "Agent Skills for Context Engineering main - execute primary workflow"

# Blast across multiple models
tcc blast "task1 for context" "task2 for engineering" "task3 for agent"
```
...
```yaml
# .claude/skills/Agent-Skills-for-Context-Engineering-main/config.yaml
skill:
  name: Agent-Skills-for-Context-Engineering-main
  version: "1.0.0"
  triggers:
    - "context"
    - "engineering"
...
User Intent
    │
    ▼
Skill Router ──► Trigger Match ──► Agent-Skills-for-Context-Engineering-main
                                       │
                    ┌──────────────────┼──────────────────┐
                    ▼                  ▼                   ▼
...
```json
{
  "nodes": [
    {
      "name": "Agent-Skills-for-Context-Engineering-main Trigger",
    ...
    },
    {
      ...
      "parameters": {
        ...
        "body": {{"skill": "Agent-Skills-for-Context-Engineering-main", "input": "{{$json.body}}"}}
      }
    }
  ]
}
...
def run_skill(task: str) -> str:
    result = subprocess.run(
        ['claude', '-p', f'using Agent-Skills-for-Context-Engineering-main skill: {task}'],
        capture_output=True, text=True
    )
    return result.stdout

output = run_skill("context analysis for my project")
print(output)
```
...
1. Fork this repo
2. Create feature branch: `git checkout -b feature/context-improvement`
3. Add/update `SKILL.md` with new capabilities
4. Test: `claude -p "test Agent-Skills-for-Context-Engineering-main skill"`
...
### v1.0.0
- Initial release with core context functionality
